  ...
  defmodule InputFromWords do
    ...
    def change(changeset, _opts, _context) do
      ...
      |> Ash.Changeset.before_transaction(
        &words_split_to_new_attributes/1
      )
    end
    ...
    defp words_split_to_new_attributes(changeset) do
      ...
      with [name, price] <- String.split(words, "/", trim: true),
        num_price <- String.to_integer(String.replace(price, ",", ""))
        do
          changeset
          |> Ash.Changeset.change_new_attribute(:name, name)
          |> Ash.Changeset.change_new_attribute(:price, num_price)
        else
          _ ->
          invalid_error = Ash.Error.Changes.InvalidArgument.exception(field: :words, message: "item/14,000 You must enter in the same format.", value: words)
          changeset
