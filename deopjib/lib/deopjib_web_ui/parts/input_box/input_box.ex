defmodule DeopjibWebUI.Parts.InputBox do
  ...
  def render(assigns) do
    ...
    >
      <div class={[
        "width-full group-data-[valid=valid]/input-box:border-primary group-data-[valid=invalid]/input-box:border-red",
        @theme_class,
        @box_class
      ]}>
        <div class="h-full flex items-center justify-center">
          <input
            ...
            class={[
              "peer flex flex-1 h-full bg-none p-0 placeholder:text-body2 placeholder:text-gray200 ring-0 border-none",
              @class
            ]}
            phx-mounted={
              assigns[:"phx-mounted"]
    ...
    >
      ...
      <%= if is_integer(@min_length) || is_integer(@max_length) do %>
        <span
          id={@id <> "_length"}
          phx-update="ignore"
          class="group-data-[valid=invalid]/input-box:text-warning ml-auto"
          data-ui="input_box#current_length"
          phx-hook={@hook}
        >
          0
        </span>
