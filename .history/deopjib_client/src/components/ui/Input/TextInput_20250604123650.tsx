...
export function TextInput({ className, placeholder, rightSection, ...props }: TextInput.Props) {
  return <BaseTextInput classNames={{
    ...
    wrapper: "TextInput__wrapper group rounded-md border-1 border-primary pr-2 h-12 py-2 flex items-center justify-center",
    ...
    <InputClearButton />
    {rightSection}
    <Button theme="gray" size="md">hihi</Button>
  </>}  {...props} />;
}


function InputClearButton() {
  return <Button className="group-has-placeholder-shown:invisible px-2 w-7.5 h-full" onClick={inputClear} >
