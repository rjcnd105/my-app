...
export function TextInput({ className, placeholder, ...props }: TextInput.Props) {
  return <BaseTextInput classNames={{
    ...
    section: "TextInput__section flex items-center",
    root: "TextInput__root",
    description: "TextInput__description",
    wrapper: "TextInput__wrapper h-12 p-2 gap-2 [&_input:placeholder-shown~div>button[data-part=close]]:invisible flex items-center justify-center",
    input: "TextInput__input",
    label: "TextInput__label",

  }} className={cn("", className)} placeholder={placeholder ?? ""} rightSection={<>
    <Button data-part="close" >
      <Icon name="cross_circle" className="size-3.5 px-2 w-7.5 h-full fill-gray100" ></Icon>
    </Button>
    <Button>hihi</Button>
  </>}  {...props} />;
}
