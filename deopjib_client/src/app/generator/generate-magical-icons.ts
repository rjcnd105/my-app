...
const iconBasePath = "src/shared/ui/Icon"
const svgFolderPath = `${iconBasePath}/svgs`
const genFile = `${iconBasePath}/IconMap.gen.tsx`
const importPath = `@shared/ui/Icon/svgs`

// ../src/icons의 파일들의 이름을 기준으로 iconNames 타입 만들기
const iconNames = fs.readdirSync(path.join(process.cwd(), svgFolderPath)).map(file => file.replace('.svg', ''));


const OUTPUT_FILE = path.join(process.cwd(), genFile);
...
function generateMagicalIconMap() {
  ...
  const componentImports = iconNames
    .map(name => {
      ...
      return `import ${pascalName}Component from "${importPath}/${name}.svg";`;
    })
    .join('\n');
  ...
  const fileImports = iconNames
    .map(name => {
      const pascalName = toPascalCase(name);
      return `import ${pascalName}File from "${importPath}/${name}.svg?file";`;
    })
    .join('\n');
...
// gen version ${new Date().toISOString()}
